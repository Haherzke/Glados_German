...
class AudioTranscriber:
    ...
    DEFAULT_MODEL_PATH = resource_path("models/ASR/de_v4_large.onnx")
    DEFAULT_CONFIG_PATH = resource_path("models/ASR/de_v1_config.yaml")
    ...
    def __init__(
        ...
        # Keep labels as-is; many CTC models include the blank token (e.g. "_") in labels
        labels: list[str] = list(self.config["labels"])  # preserve order
        self.idx2token: dict[int, str] = dict(enumerate(labels))
        ...
        # Determine the CTC blank token index from labels instead of adding a new one.
        # Common conventions: "_", "<blank>", "blank". If none found, fall back to 0.
        blank_candidates = {"_", "<blank>", "blank", "<BLANK>", "<ctc_blank>"}
        found_blank_idx = None
        for i, tok in enumerate(labels):
            if tok in blank_candidates:
                found_blank_idx = i
                break
        self.blank_idx = 0 if found_blank_idx is None else found_blank_idx
    ...
    def decode_output(self, output_logits: NDArray[np.float32]) -> list[str]:
        ...
        for batch_idx in range(predicted_indices_batch.shape[0]):
            ...
            # Handle SentencePiece-style joining if applicable
            text = "".join(tokens_str_list).replace("▁", " ").strip()
    ...
    def transcribe(self, audio: NDArray[np.float32]) -> str:
        # --- NEU: Silero erwartet rohe Waveform [batch, samples], also 2D ---
        waveform = np.expand_dims(audio.astype(np.float32), axis=0)  # [1, samples]

        # Input-Namen des Modells prüfen und passend befüllen
        input_names = {i.name for i in self.session.get_inputs()}
        if "input" in input_names:
            input_dict = {"input": waveform}
        else:
            # Fallback für frühere Modelle, die Mel wollen (dein alter Pfad)
            mel_spec = self.process_audio(audio)                       # [1, n_mels, time]
            length = np.array([mel_spec.shape[2]], dtype=np.int64)
            # üblich: "audio_signal"/"length"
            input_dict = {"audio_signal": mel_spec}
