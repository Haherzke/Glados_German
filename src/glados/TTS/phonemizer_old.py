...
class Phonemizer:
    ...
    def convert_to_phonemes(self, texts: list[str], lang: str = "de") -> list[str]:
        ...
        for text in texts:
            try:
                # Direkte IPA-Phoneme von espeak-ng
                result = subprocess.run(
                    ['espeak-ng', '-q', '--ipa=2', '-v', lang, text],
                    capture_output=True, text=True, check=True, encoding='utf-8'
                )
                phonemes = result.stdout.strip() if result.stdout else ""
                
                # GLaDOS Spezialbehandlung
                if "glados" in text.lower():
                    phonemes = phonemes.replace("glados", self.phoneme_dict.get("glados", "glados"))
                
                phoneme_results.append(phonemes)
                
            except Exception as e:
                print(f"espeak-ng Fehler: {e}")
                phoneme_results.append(text)  # Fallback
        ...
        try:
            ...
            phoneme_results = []
            for text in texts:
                # Deutsche IPA-Phoneme mit espeak-ng generieren
                phonemes = espeakng.synth(text, voice='de', ipa=2)
                # IPA-Phoneme als String zurückgeben
                phoneme_results.append(phonemes)
            
            return phoneme_results
        
        except ImportError:
            # Fallback auf subprocess wenn Library nicht verfügbar
            phoneme_results = []
            for text in texts:
                try:
                    result = subprocess.run(
                        ['espeak-ng', '-q', '--ipa', '-v', 'de', text], 
                        capture_output=True, text=True, check=True
                    )
                    phoneme_results.append(result.stdout.strip())
                except FileNotFoundError:
                    raise RuntimeError("espeak-ng nicht gefunden. Bitte installieren Sie espeak-ng oder espeak-ng-python")
        ...
        if words_to_predict:
            ...
            print("🔎 Words to predict (missing in dict):", words_to_predict)
        ...
        print("🔎 Phonemes für die Eingabetexte:", ["".join(pl) for pl in phoneme_lists])
