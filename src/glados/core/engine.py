...
class GladosConfig(BaseModel):
    ...
    asr_model_path: str
    asr_config_path: str
    # Whisper-spezifische optionale Parameter
    whisper_device: str | None = None
    whisper_fast_mode: bool | None = None
    wake_word: str | None
    voice: str
...
class Glados:
    ...
    def from_config(cls, config: GladosConfig) -> "Glados":
        ...
        asr_kwargs = {
            "model_path": Path(config.asr_model_path),
            "config_path": Path(config.asr_config_path),
        }
        
        # Whisper-spezifische Parameter hinzufügen wenn verfügbar
        if config.asr_engine.lower() == "whisper":
            if config.whisper_device is not None:
                asr_kwargs["device"] = config.whisper_device
            if config.whisper_fast_mode is not None:
                asr_kwargs["fast_mode"] = config.whisper_fast_mode
