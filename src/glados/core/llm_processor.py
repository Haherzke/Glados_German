        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                try:
                    ...
                    # Detaillierte Informationen über die HTTP-Antwort sammeln
                    status_code = "unknown"
                    response_text = ""
                    response_headers = {}
                    ...
                    if hasattr(e, "response") and e.response is not None:
                        ...
                        # Response-Text sicher abrufen
                        try:
                            response_text = e.response.text[:500]  # Erste 500 Zeichen für Debugging
                        except Exception:
                            response_text = "Could not read response text"
                        
                        # Response-Headers sicher abrufen
                        try:
                    ...
                    logger.error(
                        f"LLM Processor: HTTP error {status_code} from LLM service\n"
                        f"  Exception: {e}\n"
                        f"  Request URL: {self.completion_url}\n"
                    ...
                    # Benutzerfreundliche Nachrichten basierend auf Status-Code
                    if status_code == 401:
                        error_message = "Authentication failed with my thinking module. Please check the API key."
                    elif status_code == 403:
                        error_message = "Access denied to my thinking module. Please check permissions."
                    elif status_code == 404:
                        error_message = "My thinking module endpoint was not found. Please check the configuration."
                    elif status_code == 429:
                        error_message = "My thinking module is rate limited. Please try again in a moment."
                    elif status_code == 500:
                        error_message = "My thinking module encountered an internal server error."
                    elif status_code == 503:
                        error_message = "My thinking module is currently unavailable. Please try again later."
                    else:
                        error_message = f"I received an error from my thinking module. HTTP status {status_code}."
                    
                    self.tts_input_queue.put(error_message)
